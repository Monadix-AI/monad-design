...
export const installCoreLaunchAgent = async (
  ...
  const dependencies = { ...defaultDependencies(), ...overrides };
  const launchAgent = resolveCoreLaunchAgent(dependencies.homeDirectory, dependencies.uid);
  const expected = coreLaunchAgentPlist(executablePath, stateDirectory);
  const current = await readFile(launchAgent.path, 'utf8').catch((error: NodeJS.ErrnoException) => {
  ...
  if (current) {
    let loaded = false;
    try {
      await dependencies.exec('/bin/launchctl', ['print', launchAgent.service]);
      loaded = true;
    } catch {}
    if (loaded && current === expected) return launchAgent;
    if (loaded) await dependencies.exec('/bin/launchctl', ['bootout', launchAgent.service]);
  }
  await mkdir(dirname(launchAgent.path), { recursive: true });
  const temporaryPath = `${launchAgent.path}.${process.pid}.${Date.now()}.tmp`;
  await writeFile(temporaryPath, expected, { mode: 0o644 });
...
export const isCoreLaunchAgentLoaded = async (overrides: Partial<LaunchAgentDependencies> = {}) => {
  const dependencies = { ...defaultDependencies(), ...overrides };
  const launchAgent = resolveCoreLaunchAgent(dependencies.homeDirectory, dependencies.uid);
  try {
    await dependencies.exec('/bin/launchctl', ['print', launchAgent.service]);
    ...
    if (isMissingService(error) || /could not find service/i.test(String((error as { stderr?: string }).stderr ?? '')))
      return false;
    throw error;
  }
};
