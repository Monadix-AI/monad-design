...
On a successful start, retain the returned `uiUrl` and inspect `uiTarget`. If `uiTarget` is `desktop`, Core has opened the desktop app: do not open any browser, including the agent's built-in browser. If `uiTarget` is `browser` or `unavailable`, open `uiUrl` using the agent's available browser-opening capability. If `uiTarget` is absent (older Core), do not automatically open a browser because Desktop may already be open. Always emit `uiUrl` as a clickable Markdown link, using the exact clean localhost URL without adding query parameters. If browser opening fails, still emit the link and continue the MCP session.
